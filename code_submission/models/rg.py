...
class RidgeModel:
    ...
    def fit(self, X, y, categories):
        ...
        X_sample = X.iloc[:self.train_shape].sample(frac=0.8, random_state=2019)
        ...
        try:
            if not self.do_sample:
                ...
                if self.do_lsqr:
                    self.model = Ridge(solver='lsqr', max_iter=500, alpha=self.best_alpha)
                else:
                    self.model = Ridge(solver='svd', max_iter=300, alpha=self.best_alpha)
    ...
    def select_cols(self,  X_train, X_eval, y_train, y_eval):
        model = Ridge(solver='svd', max_iter=300, alpha=1)
    ...
    def select_alpha(self, X_train, X_eval, y_train, y_eval):
        ...
        for i in np.arange(1, 4, 1):
            model = Ridge(solver='svd', max_iter=300, alpha=i)
            model.fit(X_train, y_train)
            preds = model.predict(X_eval)
            rmse = get_rmse(preds, y_eval)
